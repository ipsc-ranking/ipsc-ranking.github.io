...
# Import division_normalizer and name_normalizer from current directory
...
class IPSCRankingSystem:
    ...
    def get_player_id(self, first_name, last_name, region, division=None, alias=None):
        """Create a unique player identifier including division with name normalization"""
        # Normalize names before creating ID
        normalized_first = normalize_name(first_name) if first_name else ""
        normalized_last = normalize_name(last_name) if last_name else ""
        
        return get_normalized_player_id(normalized_first, normalized_last, region, division)
    ...
    def process_match(self, match_data):
        ...
        match_level = match_data.get('match_level', 2)
        ...
        try:
            ...
            for i, player_id in enumerate(player_ids):
                ...
                # Track match performances for consistency calculation
                if 'match_performances' not in self.players[player_id]:
                    self.players[player_id]['match_performances'] = []
                self.players[player_id]['match_performances'].append(match_percentages[i])
...
def main():
    ...
    # Group rankings by division and category
    from collections import defaultdict
    division_rankings = defaultdict(list)
    division_category_rankings = defaultdict(lambda: defaultdict(list))
    
    # Build player-category mapping from match data
    player_categories = defaultdict(set)
    for match in matches:
        results = match.get('combined_results', match.get('shooters', []))
        for result in results:
            if 'category' in result and result['category']:
                categories = result['category'] if isinstance(result['category'], list) else [result['category']]
                division = normalize_division_name(result.get('division', 'Unknown'))
                ...
                for cat in categories:
                    if cat and cat != '-':
                        player_categories[player_id].add(cat.lower())
    ...
    for player in rankings:
        ...
        if len(parts) >= 4:
            ...
            # Add to category-specific rankings
    ...
    category_map = {
        ...
        'gs': 'grand_senior',
        'grand_senior': 'grand_senior',
        'ls': 'lady_senior',
        'lady_senior': 'lady_senior'
    }
    
    for division in division_files.keys():
        if division in division_category_rankings:
            for raw_category, category_players in division_category_rankings[division].items():
                # Normalize category name
                normalized_category = category_map.get(raw_category.lower(), raw_category.lower())
                
                # Skip if not a recognized category
                if normalized_category not in category_map.values():
                    continue
                ...
                # Filter for Swedish players and add ranks
                swedish_category_players = [p for p in category_players if p.get('region') == 'SWE']
                ...
                # Create filename
                division_normalized = division.lower().replace(' ', '_')
                category_filename = f'ipsc_ranking_{division_normalized}_{normalized_category}.json'
                ...
                print(f"✓ Saved {filepath} ({len(swedish_category_players)} Swedish players)")
    
    # Save detailed match data
    match_count = ranking_system.save_match_details('match_details.json')
    
    # Copy results to rankings directory for website
    import shutil
    os.makedirs('rankings/data', exist_ok=True)
    
    print(f"\nCopying results to rankings/data/ for website...")
    for file in os.listdir('results'):
        if file.endswith('.json'):
            shutil.copy(f'results/{file}', f'rankings/data/{file}')
            print(f"✓ Copied {file}")
    ...
    print(f"\nNext steps:")
    print(f"1. Update metadata: python update_rankings.py")
    print(f"2. Test: jekyll serve")
    print(f"3. Deploy: git commit && git push")
