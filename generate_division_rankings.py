...
def generate_all_division_rankings():
    """Generate rankings for each division using handgun-filtered data"""
    ...
    # Process all matches to build player ratings
    processed_matches = 0
    ...
    # Split into divisions and categories
    ...
    for player in overall_ranking:
        ...
        # Find all divisions and categories this player competed in
        player_divisions = set()
        player_division_data = defaultdict(lambda: {'matches': 0, 'categories': set()})
        
        # Go through all matches to find player's divisions and categories
        for match in matches:
            results = match.get('combined_results', match.get('shooters', []))
            for result in results:
                result_player_id = ranking_system.get_player_id(
                    result['first_name'],
                    result['last_name'], 
                    result.get('region'),
                    result.get('alias')
                ...
                if result_player_id == player_id:
                    ...
                    player_divisions.add(division)
                    player_division_data[division]['matches'] += 1
                    player_division_data[division]['categories'].add(category)
        
        # Create division-specific entries
        for division in player_divisions:
            division_player = player.copy()
            division_player['division'] = division
            division_player['division_matches'] = player_division_data[division]['matches']
            division_player['categories'] = list(player_division_data[division]['categories'])
            ...
            for category in player_division_data[division]['categories']:
    ...
    for division in division_files.keys():
        if division in division_category_rankings:
            for category in division_category_rankings[division]:
                ...
                # Recalculate Swedish-only category ranks and percentages
    ...
    # Recalculate Swedish-only combined ranks and percentages
